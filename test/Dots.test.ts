...
describe("Dots.play", () => {
  ...
  it("tallies squares per address across multiple submitters", () => {
    ...
    expect(dots.play([2, 0], [1, 0], ALICE).squaresClosed).toBe(0); // sq1, neutral
  ...
  it("ends in a draw when the top square count is tied", () => {
    ...
    dots.play([1, 2], [2, 2], ALICE); // 0 closed -> flip to BOB
    dots.play([2, 2], [2, 1], BOB); // 0 closed -> flip to ALICE
    dots.play([1, 1], [1, 0], ALICE); // 0 closed -> flip to BOB
    dots.play([2, 1], [2, 0], BOB); // 0 closed -> flip to ALICE
    dots.play([1, 1], [2, 1], ALICE); // 0 closed -> flip to BOB
    expect(dots.play([2, 0], [1, 0], BOB).squaresClosed).toBe(1); // closes sq1 -> BOB, turn stays
    dots.play([0, 0], [0, 1], BOB); // 0 closed -> flip to ALICE
    dots.play([1, 0], [0, 0], ALICE); // 0 closed -> flip to BOB
    expect(dots.play([0, 1], [1, 1], BOB).squaresClosed).toBe(1); // closes sq0 -> BOB, turn stays
    dots.play([0, 2], [1, 2], BOB); // 0 closed -> flip to ALICE
    expect(dots.play([1, 2], [1, 1], ALICE).squaresClosed).toBe(1); // closes sq3 -> ALICE, turn stays
    const closing = dots.play([0, 1], [0, 2], ALICE); // closes sq2 -> ALICE, fills the grid
...
describe("Dots.moveLog", () => {
  it("records a full MoveRecord per applied move, with sequencing", () => {
    ...
    dots.play([0, 1], [1, 1], BOB);
    dots.play([1, 1], [1, 0], ALICE);
    const closing = dots.play([1, 0], [0, 0], BOB); // closes the square, game ends
...
describe("Dots extra move on close", () => {
  it("closing exactly one square keeps the turn with the same player", () => {
    ...
    dots.play([0, 0], [0, 1], ALICE); // 0 closed -> flip to BOB
    dots.play([1, 0], [0, 0], BOB); // 0 closed -> flip to ALICE
    dots.play([1, 0], [1, 1], ALICE); // 0 closed -> flip to BOB
    const closing = dots.play([0, 1], [1, 1], BOB); // closes sq0

    expect(closing.squaresClosed).toBe(1);
    expect(dots.turn).toBe(BOB); // extra move: turn did not flip

    // BOB can immediately play again on the strength of the extra move.
    expect(() => dots.play([1, 1], [2, 1], BOB)).not.toThrow();
  });
  ...
  it("closing two squares in one move (double-cross) keeps the turn", () => {
    ...
    dots.play([0, 0], [0, 1], ALICE); // sq0 border, 0 closed -> flip BOB
    dots.play([1, 1], [2, 1], BOB); // sq1 border, 0 closed -> flip ALICE
    dots.play([0, 1], [1, 1], ALICE); // sq0 border, 0 closed -> flip BOB
    dots.play([2, 1], [2, 0], BOB); // sq1 border, 0 closed -> flip ALICE
    dots.play([1, 0], [0, 0], ALICE); // sq0 border, 0 closed -> flip BOB
    dots.play([2, 0], [1, 0], BOB); // sq1 border, 0 closed -> flip ALICE

    // The shared edge completes both squares at once.
    const closing = dots.play([1, 0], [1, 1], ALICE);
  ...
  it("chains multiple extra moves for the same player across independent squares", () => {
    ...
    dots.play([0, 0], [0, 1], ALICE); // sq0, 0 closed -> flip BOB
    dots.play([1, 1], [2, 1], BOB); // sq3, 0 closed -> flip ALICE
    dots.play([1, 0], [1, 1], ALICE); // sq0, 0 closed -> flip BOB
    dots.play([1, 2], [2, 2], BOB); // sq3, 0 closed -> flip ALICE
    dots.play([1, 0], [0, 0], ALICE); // sq0, 0 closed -> flip BOB
    dots.play([2, 2], [2, 1], BOB); // sq3, 0 closed -> flip ALICE

    dots.play([0, 1], [1, 1], ALICE); // closes sq0 -> turn stays ALICE
    dots.play([1, 2], [1, 1], ALICE); // closes sq3 -> turn stays ALICE (2nd extra move)
  ...
  it("still rejects the opponent moving immediately after a square-closing move", () => {
    ...
    dots.play([1, 0], [0, 0], BOB); // 0 closed -> flip to ALICE
    dots.play([1, 0], [1, 1], ALICE); // 0 closed -> flip to BOB
    dots.play([0, 1], [1, 1], BOB); // closes sq0 -> turn stays BOB

    expect(() => dots.play([1, 1], [2, 1], ALICE)).toThrow(/not this player's turn/i);
    expect(dots.turn).toBe(BOB);
...
describe("Dots longest chain (king of the chain)", () => {
  it("records a single-square close as a chain of 1", () => {
    ...
    dots.play([0, 1], [1, 1], BOB);
    dots.play([1, 1], [1, 0], ALICE);
    dots.play([1, 0], [0, 0], BOB); // BOB closes the only square
  ...
  it("accumulates a double-cross close as a chain of 2, not two separate 1s", () => {
    ...
    dots.play([0, 0], [0, 1], ALICE);
    dots.play([1, 1], [2, 1], BOB);
    dots.play([0, 1], [1, 1], ALICE);
    dots.play([2, 1], [2, 0], BOB);
    dots.play([1, 0], [0, 0], ALICE);
    dots.play([2, 0], [1, 0], BOB);
    dots.play([1, 0], [1, 1], ALICE); // closes sq0 and sq1 at once
  ...
  it("sums consecutive extra-move closes into one chain, reused from the extra-move-chain scenario", () => {
    ...
    dots.play([1, 0], [0, 0], ALICE);
    dots.play([2, 2], [2, 1], BOB);

    dots.play([0, 1], [1, 1], ALICE); // closes sq0 (chain: 1) -> turn stays ALICE
    dots.play([1, 2], [1, 1], ALICE); // closes sq3 (chain: 2) -> turn stays ALICE
  ...
  it("keeps a player's max across separate, turn-interrupted chains rather than summing them", () => {
    ...
    dots.play([1, 0], [2, 0], ALICE); // sq1 border, 0 closed -> flip BOB
    dots.play([2, 0], [2, 1], BOB); // sq1 border, 0 closed -> flip ALICE
    dots.play([1, 1], [2, 1], ALICE); // sq1 border, 0 closed -> flip BOB
    dots.play([0, 1], [0, 2], BOB); // sq2 border, 0 closed -> flip ALICE
    expect(dots.play([1, 0], [1, 1], ALICE).squaresClosed).toBe(1); // closes sq1, chain: 1 -> turn stays ALICE
    ...
    dots.play([0, 0], [0, 1], ALICE); // sq0 left, 0 closed -> flip BOB (chain reset)
    dots.play([0, 2], [1, 2], BOB); // sq2 bottom, 0 closed -> flip ALICE
    dots.play([1, 0], [0, 0], ALICE); // sq0 top, 0 closed -> flip BOB
    dots.play([1, 2], [2, 2], BOB); // sq3 bottom, 0 closed -> flip ALICE
    expect(dots.play([0, 1], [1, 1], ALICE).squaresClosed).toBe(1); // closes sq0, chain: 1 -> turn stays ALICE
  ...
  it("captures the final move's chain even though it never flips the turn", () => {
    ...
    dots.play([0, 1], [1, 1], BOB);
    dots.play([1, 1], [1, 0], ALICE);
    const closing = dots.play([1, 0], [0, 0], BOB); // closes the last square, match ends, turn never flips
